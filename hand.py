    ...
    try:
        ...
        if defects is not None:
            ...
            if cnt > 0:
                cnt = cnt+1    
            if(cnt == 0):
                pyautogui.click(stop[0],stop[1],interval=0.25)
                time.sleep(0.1)
            elif(cnt == 2):
                pyautogui.click(fata[0],fata[1],interval=0.25)
                time.sleep(0.1)
            elif(cnt == 3):
                pyautogui.click(stanga[0],stanga[1],interval=0.25)
                time.sleep(0.1)
            elif(cnt == 4):
                pyautogui.click(dreapta[0],dreapta[1],interval=0.25)
                time.sleep(0.1)
            elif(cnt == 5):
                pyautogui.click(spate[0],spate[1],interval=0.25)
                time.sleep(0.1)
...
while cap.isOpened():
    ...
    frame[upper_left[1] : bottom_right[1], upper_left[0] : bottom_right[0]] = hand(ROI) 
    cv2.imshow('webcam', frame )
    cv2.imshow('color',color_detection(ROI))
    cv2.imshow('img',hand(ROI))
